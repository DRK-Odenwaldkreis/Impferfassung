...
logger = logging.getLogger(f'Forecast creation started on: {datetime.datetime.now()}')
...
if __name__ == "__main__":
    try:
        ...
        for i in vaccine:
            ...
            PDF = PDFgenerator(doses,booster,ages,waiting,i[1], requestedDate)
            filename = PDF.generate()
            if send:
                logger.debug('Sending Mail')
                send_mail_forecast(filename,requestedDate,get_Leitung_from_StationID(0))
        logger.info('Done')
    except Exception as e:
