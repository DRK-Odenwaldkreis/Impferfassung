...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) == 2:
            ...
            sql = "Select Voranmeldung.id,Nachname,Vorname,Telefon,Mailadresse,Impfstoff.Kurzbezeichnung,Voranmeldung.Tag,Termine.Stunde,Termine.Slot from Voranmeldung LEFT JOIN Termine ON Termine.id=Voranmeldung.Termin_id LEFT JOIN Station ON Station.id=Termine.id_station LEFT JOIN Impfstoff ON Impfstoff.id=Station.Impfstoff_id where Voranmeldung.Tag Between '%s 00:00:00' and '%s 23:59:59' ORDER BY Termine.Stunde,Termine.Slot;" % (requestedDate.replace('-', '.'), requestedDate.replace('-', '.'))
        else:
